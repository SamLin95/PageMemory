...
double compute_emat() {
   ...
   long int total_accesses = count_writes + count_reads;
   long int tlb_misses = total_accesses - count_tlbhits;
   long int hits = count_tlbhits * MEMORY_ACCESS_TIME;
   long int misses = tlb_misses * 2 * MEMORY_ACCESS_TIME;
   long int fault = count_pagefaults * DISK_ACCESS_TIME;
   return (hits + misses + fault) / ((double)total_accesses);
}
